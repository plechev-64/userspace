...
function usp_user_posts() {
	...
	if ( false !== array_search( 'posts_count', $usp_users_set->data ) || isset( $usp_user->posts_count ) ) {
		...
		echo '<span class="filter-data"><i class="uspi fa-file-text-o"></i>' . __( 'Publics', 'usp' ) . ': ' . $usp_user->posts_count . '</span>';
	}
}
...
function usp_user_action( $type = 1 ) {
	...
	switch ( $type ) {
		case 1: $last_action = usp_get_useraction( $action );
			if ( ! $last_action )
				echo '<span class="status_user online"><i class="uspi fa-ciuspe"></i></span>';
			else
				echo '<span class="status_user offline" title="' . __( 'offline', 'usp' ) . ' ' . $last_action . '"><i class="uspi fa-ciuspe"></i></span>';
			break;
...
function usp_user_register() {
	...
	if ( false !== array_search( 'user_registered', $usp_users_set->data ) || isset( $usp_user->user_registered ) ) {
		if ( ! isset( $usp_user->user_registered ) )
			return false;
		echo '<span class="filter-data"><i class="uspi fa-calendar-check-o"></i>' . __( 'Registration', 'usp' ) . ': ' . mysql2date( 'd-m-Y', $usp_user->user_registered ) . '</span>';
	}
}
...
function usp_get_useraction_html( $user_id, $type = 1 ) {
	...
	switch ( $type ) {
		case 1:
			...
			if ( ! $last_action )
				return '<span class="status_user online"><i class="uspi fa-ciuspe"></i></span>';
			else
				return '<span class="status_user offline" title="' . __( 'offline', 'usp' ) . ' ' . $last_action . '"><i class="uspi fa-ciuspe"></i></span>';
...
function usp_get_miniaction( $action ) {
	...
	$content .= ( ! $last_action && $action) ? '<i class="uspi fa-ciuspe"></i>' : __( 'offline', 'usp' ) . ' ' . $last_action;
