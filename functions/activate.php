...
if ( ! class_exists( 'reg_core' ) ) {
	...
	class reg_core {
		function __construct() {
			...
			add_action( 'usp_cron_daily', array( &$this, 'chekplugs' ), 10 );
		}
		...
		function remote( $dir, $data = array() ) {
			...
			$response	 = wp_remote_post( USP_SERVICE_HOST . '/activate-plugins/access/2.0/' . $dir . '/?plug=' . $_GET['plug'], array( 'body' => $data ) );
			if ( ! $response['body'] )
				return false;
	...
	function reg_form_wpp( $id, $path = false ) {
		...
		if ( get_site_option( WP_PREFIX . $id ) == WP_HOST ) {
			...
			if ( $_GET['id_access_' . $id] ) {
				switch ( $_GET['id_access_' . $id] ) {
					...
					case 8: $content .= '<div class="error"><p>Переданы неверные данные</p></div>';
						break;
					case 9: $content .= '<div class="error"><p>Для вашего домена действует другой ключ <a href="' . USP_SERVICE_HOST . '/activate-plugins/findkey/?plug=' . $id . '&host=' . $_SERVER['HTTP_HOST'] . '">Потеряли ключ?</a></p></div>';
						break;
				}
			}
			...
			$content .= '<div class="error"><p>Плагин не активирован!</p></div>'
				. '<h3>Введите ключ:</h3>
                <form action="' . USP_SERVICE_HOST . '/activate-plugins/access/2.0/gk/?plug=' . $id . '" method="post">
