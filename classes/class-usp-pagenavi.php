...
class USP_PageNavi {
    ...
    public $key           = 'usp-page'; //ключ передаваемый номер текущей страницы
    ...
    function __construct( $pager_id, $data_amount, $custom = array() ) {
        ...
        if ( $usp_tab ) {
            if ( in_array( 'ajax', $usp_tab->supports ) ) {
                $this->ajax = true;
            }
        }
    ...
    function __destruct() {
        remove_all_filters( 'usp_page_link_attributes' );
    }
    ...
    function get_url( $page_id ) {
        return usp_format_url( $this->uri['current'] ) . $this->uri['string'] . '&' . $this->key . '=' . $page_id;
    }

    function pagenavi( $classes = '' ) {
        global $usp_tab, $user_LK;
        ...
        foreach ( $query['output'] as $item ) {
            foreach ( $item as $type => $data ) {
                if ( $type == 'page' ) {
                    ...
                    if ( $this->ajax && usp_is_office() ) {

                        $attrs['data']['post'] = usp_encode_post( array(
                            'tab_id'    => $usp_tab->id,
                            'subtab_id' => $usp_tab->active_subtab,
                            'master_id' => $user_LK,
                            'page'      => $attrs['data']['page'],
                            ) );

                        $attrs['class'] = 'usp-ajax';
                    }

                    $attrs = apply_filters( 'usp_page_link_attributes', $attrs );

                    $html = usp_get_button( $attrs );
