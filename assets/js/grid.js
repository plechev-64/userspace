/**
 * Инициализирует один компонент грида (таблицы с данными).
 * @param {HTMLElement} gridContainer - DOM-элемент контейнера грида (.usp-grid-container).
 */
function initGrid(gridContainer) {
    // Предотвращаем повторную инициализацию
    if (gridContainer.dataset.uspGridInitialized) {
        return;
    }
    gridContainer.dataset.uspGridInitialized = 'true';
    ...
    if (!endpoint) {
        console.error('Grid initialization error: data-endpoint attribute is missing.', gridContainer);
        return;
    }
    ...
    const fetchData = async (page = 1, search = '', orderby = 'id', order = 'desc') => {
        ...
        try {
            ...
            if (gridContainer.classList.contains('usp-table-grid-container')) {
    ...
    if (paginationContainer) {
        paginationContainer.addEventListener('click', (e) => {
            ...
            const page = parseInt(url.pathname.split('/').filter(Boolean).pop(), 10) || 1;
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.usp-grid-container').forEach(initGrid);
});

// 2. Инициализация для гридов, загруженных динамически через REST.
document.addEventListener('usp:tabContentLoaded', (event) => {
    event.detail.pane.querySelectorAll('.usp-grid-container').forEach(initGrid);
});