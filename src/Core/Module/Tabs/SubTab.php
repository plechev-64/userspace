...
namespace USP\Core\Module\Tabs;
...
class SubTab {
	...
	public function get_content(): string {
		...
		if ( $this->callback ) {
			...
			if(str_contains($this->callback['name'], '::')){
				$methodClass = explode('::', $this->callback['name']);
				$className = $methodClass[0];
				$methodName = $methodClass[1];
				$contentTab = (new $className())->$methodName( ...$args);
			}else{
				if ( function_exists( $this->callback['name'] ) ) {
					$contentTab = call_user_func_array( $this->callback['name'], $args );
				} else {
					$contentTab = usp_get_notice( [ 'text' => __( 'There was an error loading the tab. Function not found.', 'userspace' ) ] );
				}
			}

			$content .= '<div class="usp-subtab-content">';
			$content .= apply_filters( 'usp_tab_content', $contentTab, $this->parent_id, $this->id );
			$content .= '</div>';
		}
