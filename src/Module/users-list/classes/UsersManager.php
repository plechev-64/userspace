...
class UsersManager extends ContentManager {

	private array $_required_params = [
		'number'           => 10,
	...
	private function enqueue_assets(): void {
		...
		if ( ! $this->get_param( 'style_in_content' ) && in_array( $this->get_param( 'template' ), [
			...
			usp_enqueue_style(
				...
				USP_URL . 'src/Module/users-list/assets/css/usp-users-' . $this->get_param( 'template' ) . '.css'
			);
		}

	}

	public function get_query(): UsersQuery {
	...
	private function join_last_activity( UsersQuery $query ): UsersQuery {
	...
	private function join_posts( QueryBuilder $query ): QueryBuilder {
	...
	private function join_comments( QueryBuilder $query ): QueryBuilder {
	...
	public function filter_items( array $users ): array {
	...
	public function get_items_content(): string {
		...
		if ( ! $items ) {
			...
			if ( $this->get_param( 'style_in_content' ) && in_array( $this->get_param( 'template' ), [
				...
				$content .= '<link rel="stylesheet" href="' . USP_URL . 'src/Module/users-list/assets/css/usp-users-' . $this->get_param( 'template' ) . '.css' . '">';
			}
