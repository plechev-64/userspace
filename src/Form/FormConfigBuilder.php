...
class FormConfigBuilder {
	...
	/**
	 * Возвращает собранный конфигурационный массив.
	 *
	 * @return array
	 */
	 ...
	 *
	 * @param array $config Конфигурация для загрузки.
	 *
	 * @return $this
	 */
	public function load( array $config ): self {
		...
		foreach ( $config['sections'] as $section_index => $section ) {
			...
			foreach ( $section['blocks'] as $block_index => $block ) {
				$block_id = $block['id'] ?? 'block-' . $block_index;
				$this->addBlock( $block_id, $block['title'] ?? '' );
